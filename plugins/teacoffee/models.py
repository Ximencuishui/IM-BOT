...
class TeaProduct(Base):
    ...
    unit = Column(String(20), default='杯')
    description = Column(Text)
    image_url = Column(String(500))
    is_available = Column(TinyInteger, default=1)
    stock = Column(Integer, default=0)
    sort_order = Column(Integer, default=0)
    sugar_levels = Column(JSON)
    ice_levels = Column(JSON)
    size_options = Column(JSON)
    ...
    def to_dict(self):
        return {
            ...
            'sugar_levels': json.loads(self.sugar_levels) if self.sugar_levels else [],
            'ice_levels': json.loads(self.ice_levels) if self.ice_levels else [],
...
class TeaOrderItem(Base):
    __tablename__ = 't_tea_order_item'

    id = Column(Integer, primary_key=True, autoincrement=True)
    order_id = Column(Integer, ForeignKey('t_tea_order.id'), nullable=False)
    product_id = Column(Integer, ForeignKey('t_tea_product.id'), nullable=False)
    product_name = Column(String(200))
    quantity = Column(Integer, default=1)
    unit_price = Column(Numeric(10, 2), default=0.00)
    total_price = Column(Numeric(10, 2), default=0.00)
    sugar_level = Column(String(20))
    ice_level = Column(String(20))
...
class TeaCategory(Base):
    __tablename__ = 't_tea_category'

    id = Column(Integer, primary_key=True, autoincrement=True)
    category_name = Column(String(100), nullable=False)
    category_code = Column(String(50))
    parent_id = Column(Integer, ForeignKey('t_tea_category.id'))
    sort_order = Column(Integer, default=0)
...
class TeaMember(Base):
    __tablename__ = 't_tea_member'
...
class TeaIngredient(Base):
    __tablename__ = 't_tea_ingredient'
...
class TeaSalesRecord(Base):
    __tablename__ = 't_tea_sales_record'
    ...
    popular_product = Column(String(200))
    ...
    def to_dict(self):
        return {
            ...
            'popular_product': self.popular_product,

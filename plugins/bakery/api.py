...
from plugins.bakery.service import BakeryService

logger = logging.getLogger(__name__)

bakery_bp = Blueprint('bakery', __name__, url_prefix='/api/bakery')


@bakery_bp.route('/products', methods=['GET', 'POST'])
@login_required
def products():
    db: Session = get_db_session()
    service = BakeryService(db)
...
@bakery_bp.route('/products/<int:product_id>', methods=['GET', 'PUT', 'DELETE'])
@login_required
...
@bakery_bp.route('/orders', methods=['GET', 'POST'])
@login_required
def orders():
    db: Session = get_db_session()
    service = BakeryService(db)
    ...
    if request.method == 'GET':
        ...
        is_custom_order = request.args.get('is_custom_order', None)
        if is_custom_order is not None:
            is_custom_order = int(is_custom_order)
        orders = service.list_orders(status, order_type, is_custom_order)
        db.close()
    ...
    if not data:
        db.close()
        return jsonify({'success': False, 'error': '缺少订单数据'}), 400
...
@bakery_bp.route('/orders/<int:order_id>', methods=['GET', 'PUT'])
@login_required
...
@bakery_bp.route('/categories', methods=['GET', 'POST'])
@login_required
def categories():
    db: Session = get_db_session()
    service = BakeryService(db)
...
@bakery_bp.route('/members', methods=['GET', 'POST'])
@login_required
def members():
    db: Session = get_db_session()
    service = BakeryService(db)
...
@bakery_bp.route('/members/phone/<phone>', methods=['GET'])
@login_required
...
@bakery_bp.route('/ingredients', methods=['GET', 'POST'])
@login_required
def ingredients():
    db: Session = get_db_session()
    service = BakeryService(db)
...
@bakery_bp.route('/parse_message', methods=['POST'])
@login_required
def parse_message():
    db: Session = get_db_session()
    service = BakeryService(db)
...
@bakery_bp.route('/report', methods=['GET'])
@login_required
def report():
    db: Session = get_db_session()
    service = BakeryService(db)

...
from plugins.hotpot.service import HotpotService

logger = logging.getLogger(__name__)

hotpot_bp = Blueprint('hotpot', __name__, url_prefix='/api/hotpot')


@hotpot_bp.route('/products', methods=['GET', 'POST'])
@login_required
def products():
    db: Session = get_db_session()
    service = HotpotService(db)
...
@hotpot_bp.route('/products/<int:product_id>', methods=['GET', 'PUT', 'DELETE'])
@login_required
...
@hotpot_bp.route('/orders', methods=['GET', 'POST'])
@login_required
def orders():
    db: Session = get_db_session()
    service = HotpotService(db)
    ...
    if request.method == 'GET':
        ...
        pot_type = request.args.get('pot_type', None)
        orders = service.list_orders(status, order_type, pot_type)
        db.close()
...
@hotpot_bp.route('/orders/<int:order_id>', methods=['GET', 'PUT'])
@login_required
...
@hotpot_bp.route('/categories', methods=['GET', 'POST'])
@login_required
def categories():
    db: Session = get_db_session()
    service = HotpotService(db)
...
@hotpot_bp.route('/pot_types', methods=['GET', 'POST'])
@login_required
def pot_types():
    db: Session = get_db_session()
    service = HotpotService(db)
    ...
    if request.method == 'GET':
        pot_types = service.list_pot_types()
        db.close()
        return jsonify({'success': True, 'count': len(pot_types), 'pot_types': pot_types}), 200

    data = request.get_json()
    if not data or 'pot_name' not in data:
        db.close()
        return jsonify({'success': False, 'error': '缺少锅底名称'}), 400

    result = service.create_pot_type(data)
    db.close()
    return jsonify(result), 201 if result['success'] else 400


@hotpot_bp.route('/parse_message', methods=['POST'])
@login_required
def parse_message():
    db: Session = get_db_session()
    service = HotpotService(db)
...
@hotpot_bp.route('/report', methods=['GET'])
@login_required
def report():
    db: Session = get_db_session()
    service = HotpotService(db)
...
@hotpot_bp.route('/stats', methods=['GET'])
@login_required
def stats():
    db: Session = get_db_session()
    service = HotpotService(db)

...
class HardwareProduct(Base):
    __tablename__ = 't_hardware_product'
...
class HardwareOrder(Base):
    __tablename__ = 't_hardware_order'
    ...
    install_required = Column(TinyInteger, default=0)
...
class HardwareOrderItem(Base):
    __tablename__ = 't_hardware_order_item'

    id = Column(Integer, primary_key=True, autoincrement=True)
    order_id = Column(Integer, ForeignKey('t_hardware_order.id'), nullable=False)
    product_id = Column(Integer, ForeignKey('t_hardware_product.id'), nullable=False)
...
class HardwareCategory(Base):
    __tablename__ = 't_hardware_category'

    id = Column(Integer, primary_key=True, autoincrement=True)
    category_name = Column(String(100), nullable=False)
    category_code = Column(String(50))
    parent_id = Column(Integer, ForeignKey('t_hardware_category.id'))
    sort_order = Column(Integer, default=0)
...
class HardwareCustomer(Base):
    __tablename__ = 't_hardware_customer'
...
class HardwareInstallService(Base):
    __tablename__ = 't_hardware_install_service'
...
class HardwareSalesRecord(Base):
    __tablename__ = 't_hardware_sales_record'
    ...
    install_orders = Column(Integer, default=0)
    install_amount = Column(Numeric(12, 2), default=0.00)
    created_at = Column(DateTime, default=datetime.now)
    ...
    def to_dict(self):
        return {
            ...
            'install_orders': self.install_orders or 0,
            'install_amount': float(self.install_amount) if self.install_amount else 0.00,
            'created_at': self.created_at.isoformat() if self.created_at else None,
        }